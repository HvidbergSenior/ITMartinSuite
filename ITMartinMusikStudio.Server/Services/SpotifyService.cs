...
// OAuth Authorization Code flow (not PKCE) - safe to keep the client secret
// server-side since this whole app is Blazor Server, never a public SPA.
// Tokens persist to a small JSON file next to the SQLite DB (same low-tech
// pattern this project already uses for schema changes) since this is a
// single-user personal tool, not multi-tenant - no need for a real token
// store per user.
public sealed class SpotifyService
{
    ...
    public SpotifyService(IHttpClientFactory httpFactory, IConfiguration config)
    {
        ...
        var dataRoot = Path.GetDirectoryName(
            (config.GetConnectionString("StudioDb") ?? "Data Source=/app/data/studio.db")
                .Replace("Data Source=", ""))
            ?? "/app/data";

...
public class MediaRulesWorkflowStepTests
{
    ...
    public void SetUp()
    {
        _root = Path.Combine(Path.GetTempPath(), "Package1MediaRulesTests_" + Guid.NewGuid().ToString("N"));
    ...
    private async Task<MediaFile> ClassifyAsync(string fileName, bool withAudioSibling)
    {
        ...
        var state = new Package1WorkflowState { MediaFiles = [file] };
        var step = new MediaRulesWorkflowStep(Mock.Of<IVideoMetadataService>(), NullLogger<MediaRulesWorkflowStep>.Instance);
        var context = new WorkflowExecutionContext<Package1WorkflowState> { WorkflowName = "Package1Workflow", State = state };

...
// Package1 must only ever run against genuine raw source material - a folder
...
// stripped first, or Package1 would ingest _Galleri thumbnails/SmartFolders
...
public class CleanStartWorkflowStepTests
{
    ...
    public void SetUp()
    {
        _root = Path.Combine(Path.GetTempPath(), "Package1CleanStartTests_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var state = new Package1WorkflowState { RootPath = _root };

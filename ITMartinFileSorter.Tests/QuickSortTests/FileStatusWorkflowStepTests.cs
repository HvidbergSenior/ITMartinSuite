...
// be hardcoded false for every image regardless of what QuickSort actually
...
// conversation this was built from - QuickSort should get files to a
// finished (IsDone) state as fast as possible on the first run, not defer
// everything to a later FaceIndex catch-up pass.
[TestFixture]
public class FileStatusWorkflowStepTests
{
    ...
    public void SetUp()
    {
        _root = Path.Combine(Path.GetTempPath(), "QuickSortStatusTests_" + Guid.NewGuid().ToString("N"));
    ...
    [Test]
    public async Task A_real_photo_with_known_orientation_and_confirmed_quality_reaches_IsDone_from_QuickSort_alone()
    {
        // The actual point of both fixes: a normal, unremarkable photo
        // (has EXIF orientation, isn't blurry/solid) should never need a
        // later FaceIndex catch-up pass at all.
    ...
    {
        // Audio is just copied, never run through QuickSort's normalization/

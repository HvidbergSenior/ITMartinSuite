...
public sealed class ImageTaggingService : IImageTaggingService
{
    ...
    // Constructed directly rather than DI-injected - QuickSortManifestLoader/Writer
    // are only registered via AddQuickSortPipeline (the Worker's DI graph), not
    ...
    private readonly QuickSortManifestWriter _manifestWriter = new();
    ...
    // known-flaky QuickSort.razor Start button invites exactly this: a user
    ...
    private async Task<ImageTaggingResult> TagLibraryCoreAsync(string libraryPath, CancellationToken cancellationToken)
    {
        ...
        // paths (QuickSortManifestLoader does this automatically whenever
        ...
        var rawManifest = JsonSerializer.Deserialize<QuickSortManifest>(rawJson)

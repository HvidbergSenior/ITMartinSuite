...
public sealed class FaceIndexService : IFaceIndexService
{
    ...
    private readonly ILogger<FaceIndexService> _logger;

    public FaceIndexService(
        ...
        ILogger<FaceIndexService> logger)
    {
    ...
    public async Task IndexFacesAsync(string libraryPath, int? maxDatedReferenceFiles = null, CancellationToken cancellationToken = default)
    {
        ...
        var typeName = FaceIndexIndexType.Faces.ToString();
        ...
        {
            _logger.LogError(ex, "FaceIndex face indexing failed for {LibraryPath}", libraryPath);
    ...
    private async Task IndexFaceForFileAsync(MediaDbContext db, string libraryPath, string file, IFaceRecognitionService recognizer, CancellationToken cancellationToken)
    {
        ...
        if (isVideo)
        {
            // QuickSort already generated a thumbnail for every video during
    ...
    public async Task<FaceIndexIndexStatus?> GetIndexStatusAsync(string libraryPath, FaceIndexIndexType indexType)
    {
    ...
    // over the course of local FaceIndex testing (see docker-compose.yaml):
    ...
    public async Task<List<PersonMatchResult>> FindMatchesAsync(Guid personId, double threshold = 0.45)
    {
        ...
        // FaceIndex testing) - without this, the same photo shows up as two
    ...
    // "Undated"/"Ikke i årsmapper" subfolder convention - QuickSort already

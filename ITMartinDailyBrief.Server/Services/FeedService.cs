...
public class FeedService
{
    ...
    public FeedService(IHttpClientFactory factory, ILogger<FeedService> log)
    {
        _factory = factory;
        _log     = log;
    }
    ...
    {
        var tasks   = sources.Where(s => s.Enabled).Select(FetchSourceAsync);
    ...
    private async Task<List<FeedItem>> FetchSourceAsync(FeedSource source)
    {
        ...
        {
            _log.LogWarning("Feed {Name} failed: {Msg}", source.Name, ex.Message);
            return [];
        }
    }
    ...
    private static List<FeedItem> ParseFeed(XDocument doc, FeedSource source)
    {
        ...
        foreach (var el in doc.Descendants("item"))
        {
            ...
            var link  = el.Element("link")?.Value.Trim()
                     ?? el.Elements().FirstOrDefault(e => e.Name.LocalName == "origLink")?.Value ?? "";
            var desc  = StripHtml(
                el.Element(Content + "encoded")?.Value
                ?? el.Element("description")?.Value ?? "");
            var date  = ParseDate(el.Element("pubDate")?.Value ?? el.Element(Dc + "date")?.Value);
            var img   = ExtractImage(el);

            if (!string.IsNullOrEmpty(title))
                items.Add(new FeedItem { Title = title, Url = link, Description = desc,
                    ImageUrl = img, SourceName = source.Name, SourceColor = source.Color, Published = date });
        }
        ...
        if (items.Count == 0)
        {
            ...
            foreach (var el in doc.Descendants(ns + "entry"))
            {
                ...
                var link  = el.Elements(ns + "link").FirstOrDefault()?.Attribute("href")?.Value ?? "";
                var desc  = StripHtml(el.Element(ns + "content")?.Value ?? el.Element(ns + "summary")?.Value ?? "");
                var date  = ParseDate(el.Element(ns + "updated")?.Value ?? el.Element(ns + "published")?.Value);
                var img   = ExtractImage(el);
    ...
    {
        var thumb = el.Element(Media + "thumbnail")?.Attribute("url")?.Value;
        if (!string.IsNullOrEmpty(thumb)) return thumb;

        var mc = el.Elements(Media + "content")
        ...
        if (enc?.Attribute("type")?.Value.StartsWith("image") == true)
    ...
    private static DateTime ParseDate(string? raw)
    {
        ...
        if (DateTimeOffset.TryParseExact(raw,
            ...
            System.Globalization.DateTimeStyles.None, out var dto)) return dto.UtcDateTime;
